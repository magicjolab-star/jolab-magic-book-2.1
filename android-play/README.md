# Magic Book — Android / Google Play

Trusted Web Activity (TWA) pour `https://jolab-magic-book-20.vercel.app/`.

- Package Android : `com.magicjolab.magicbook`
- Version : `3.0.2` (`versionCode 30002`)
- `compileSdk` / `targetSdk` : 36
- `minSdk` : 23
- Android Browser Helper : 2.6.2

Le bundle signé est généré par GitHub Actions. La clé d'importation (upload key) doit être conservée en lieu sûr pour les futures mises à jour.
